...
[English](#english-version) | [Русская версия](#русская-версия) | [Українська версія](#українська-версія)
...
# Налаштування Ghostty + tmux (macOS)

Ця зв’язка призначена для розробників, які хочуть використовувати:

- **Ghostty** як основний dev-термінал;
- **tmux** для керування сесіями, вікнами та панелями;
- при цьому залишити системний macOS Terminal без змін (без автоматичного запуску tmux).

## Стек і поведінка

- Під час запуску **Ghostty** в інтерактивній `zsh`-сесії автоматично стартує `tmux` (сесія `main`).
- У **macOS Terminal.app** `tmux` автоматично не запускається - працює звичайний `zsh`.
- Конфіги лежать у репозиторії в папці `config/` і копіюються в `$HOME`.
...
- macOS
- [Homebrew](https://brew.sh/) (для встановлення tmux)
- [Ghostty](https://ghostty.org/) встановлено в `/Applications/Ghostty.app`
- Опційно: шрифт `JetBrainsMono Nerd Font` (або будь-який інший моноширинний шрифт)

---

## 2. Структура конфігів у репозиторії
...
-   `config/ghostty/config` - конфігурація Ghostty.
    
-   `config/tmux/tmux.conf` - основний конфіг tmux.
    
-   `config/zsh/tmux-ghostty.zsh` - сніпет для ввімкнення tmux лише в Ghostty.
    

----------

## 3. Встановити tmux через Homebrew
...
### 4.1. Основний конфіг tmux

Файл із репозиторію: `config/tmux/tmux.conf`
...
# Нумерація вікон/панелей з 1
set -g base-index 1
setw -g pane-base-index 1
set -g renumber-windows on

# Vi-режим для копіювання та навігації
setw -g mode-keys vi
...
# Перезавантаження конфіга: Alt(Option)+r
bind -n M-r source-file ~/.config/tmux/tmux.conf \; display "Config reloaded!"

# Дерево сесій/вікон: Alt(Option)+s
bind -n M-s choose-tree -s

# Швидке перемикання вікон: Alt(Option)+1..9
bind -n M-1 select-window -t 1
bind -n M-2 select-window -t 2
bind -n M-3 select-window -t 3
bind -n M-4 select-window -t 4
...
# Навігація по панелях: Alt(Option)+стрілки
bind -n M-Left  select-pane -L
bind -n M-Right select-pane -R
bind -n M-Up    select-pane -U
bind -n M-Down  select-pane -D

# Зміна розмірів панелей: Alt(Option)+Shift+стрілки
bind -n M-S-Left  resize-pane -L 5
bind -n M-S-Right resize-pane -R 5
bind -n M-S-Up    resize-pane -U 3
bind -n M-S-Down  resize-pane -D 3
...
# Спліти
bind -n M-h split-window -v   # Alt(Option)+h - горизонтальний спліт (верх/низ)
bind -n M-v split-window -h   # Alt(Option)+v - вертикальний спліт (ліворуч/праворуч)

# Вікна
bind -n M-Enter new-window    # Alt(Option)+Enter - нове вікно
bind -n M-c kill-pane         # Alt(Option)+c - закрити панель
bind -n M-q kill-window       # Alt(Option)+q - закрити вікно
bind -n M-d detach            # Alt(Option)+d - detach
bind -n M-Q confirm-before -p "Kill entire session? (y/n)" kill-session

# Копіювання у vi-режимі
bind -T copy-mode-vi v send -X begin-selection
# macOS: копіюємо в системний буфер обміну
bind -T copy-mode-vi y send -X copy-pipe-and-cancel "pbcopy"
...
# Авто-збереження/відновлення сесій
set -g @continuum-restore 'on'
set -g @continuum-save-interval '15'
...
### 4.2. `.tmux.conf` у домашній директорії

Створити файл `~/.tmux.conf`, який підключає основний конфіг:
...
### 4.3. Встановлення TPM і плагінів
...
Натиснути `Ctrl+b`, потім `Shift+I` (велика `I`), щоб встановити плагіни.

----------

## 5. Конфігурація Ghostty

Файл із репозиторію: `config/ghostty/config`
...
Перезавантаження конфіга Ghostty:
...
## 6. Автозапуск tmux лише в Ghostty (zsh)

Файл із репозиторію: `config/zsh/tmux-ghostty.zsh`

```text
# tmux only in Ghostty
if [[ $- == *i* ]]; then
  # перевіряємо, що термінал - Ghostty
  if [[ "$TERM_PROGRAM" == "ghostty" ]]; then
    if [ -z "$TMUX" ] && command -v tmux >/dev/null 2>&1; then
      tmux attach -t main || tmux new -s main
    fi
...
## 7. Поведінка після налаштування

-   Запуск Ghostty:
    
    -   стартує `zsh`,
        
    -   в інтерактивній сесії, якщо термінал - Ghostty і немає активного tmux, автоматично виконується:
        
        -   `tmux attach -t main || tmux new -s main`,
            
    -   у результаті одразу потрапляємо в tmux-сесію `main`.
        
-   Запуск Terminal.app:
    
    -   змінна `TERM_PROGRAM` відрізняється від `ghostty`,
        
    -   блок автозапуску tmux не спрацьовує,
        
    -   отримуємо звичайний `zsh` без tmux.
        

----------

## 8. Коротке використання

Нижче описано, як працювати з цією зв’язкою Ghostty + tmux у реальних сценаріях розробки.

### 8.1. Базова модель tmux

tmux оперує трьома сутностями:

-   **Сесія (session)** - логічний робочий контекст (наприклад: проєкт, сервер).
    
-   **Вікно (window)** - вкладка всередині сесії (часто відповідає “режиму роботи”: код, логи, ssh тощо).
    
-   **Панель (pane)** - поділ вікна на кілька областей екрана (терміналів).
    

Ghostty поверх цього дає нативні вікна/таби macOS, але основна логіка роботи - всередині tmux.

### 8.2. Сесії tmux

Сесія `main` створюється/підхоплюється автоматично під час запуску Ghostty.
...
# підключитися до існуючої сесії
tmux attach -t my-session
tmux attach -t main

# переключитися на іншу сесію (всередині tmux)
tmux switch-client -t my-session
...
-   `Alt(Option)+d` - detach (сесія продовжує працювати у фоні, можна підключитися пізніше).
    
-   `Alt(Option)+Q` - запит на повне завершення сесії (kill-session із підтвердженням).
    

### 8.3. Вікна (windows)

Вікна зручно використовувати як “логічні вкладки” всередині одного проєкту/сесії, наприклад:

-   вікно 1 - редактор/код,
    
-   вікно 2 - сервер/бекенд,
...
-   Нове вікно: `Alt(Option)+Enter`
    
-   Закрити вікно: `Alt(Option)+q` (додаткове підтвердження від tmux не потрібне, вікно закривається одразу)
    
-   Перемикання між вікнами:
    
    -   `Alt(Option)+1` … `Alt(Option)+9` - перехід до вікна за номером (нумерація починається з 1, пересортовується через `set -g renumber-windows on`)
        
-   Дерево сесій і вікон:
    
    -   `Alt(Option)+s` - відкрити дерево (сесії/вікна/панелі), навігація стрілками, Enter - перейти.
        

### 8.4. Панелі (panes) у вікні

Панелі - це “розділення екрана” всередині одного вікна tmux.

Створення сплітів:

-   `Alt(Option)+h` - горизонтальний спліт (верх/низ).
    
-   `Alt(Option)+v` - вертикальний спліт (ліворуч/праворуч).
    

Навігація між панелями:

-   `Alt(Option)+Left` - фокус на панель ліворуч
    
-   `Alt(Option)+Right` - фокус на панель праворуч
    
-   `Alt(Option)+Up` - фокус на панель вище
    
-   `Alt(Option)+Down` - фокус на панель нижче
    

Зміна розмірів панелей:

-   `Alt(Option)+Shift+Left` - збільшити ліву панель (зменшити праву) на 5 колонок
    
-   `Alt(Option)+Shift+Right` - збільшити праву панель (зменшити ліву) на 5 колонок
    
-   `Alt(Option)+Shift+Up` - збільшити верхню панель (зменшити нижню) на 3 рядки
    
-   `Alt(Option)+Shift+Down` - збільшити нижню панель (зменшити верхню) на 3 рядки
    

Закриття панелі:

-   `Alt(Option)+c` - закрити поточну панель (`kill-pane`)
    

### 8.5. Копіювання та буфер обміну (macOS)

tmux працює в режимах копіювання, зручно використовувати vi-навігацію.

Увійти в режим копіювання:

```bash
Alt(Option)+/   # копіювання + пошук вниз
Alt(Option)+?   # копіювання + пошук вгору

```

Або просто `Alt(Option)+/` і вручну переміщатися по історії.

У режимі `copy-mode-vi`:

-   `v` - почати виділення
    
-   стрілки / `h/j/k/l` - рухати курсор і виділення
    
-   `y` - скопіювати виділене й вийти з режиму
    

Копіювання одразу потрапляє в системний буфер обміну macOS (через `pbcopy`), і текст можна вставити в будь-який застосунок (`Cmd+V`).

### 8.6. Пошук по історії

У будь-який момент, всередині tmux:

-   `Alt(Option)+/` - увійти в copy-mode і запросити рядок для пошуку вниз.
    
-   `Alt(Option)+?` - те саме, але пошук вгору.
    

Далі:

-   введення рядка пошуку,
    
-   `n / N` у vi-режимі - перехід до наступного/попереднього збігу.
    

### 8.7. Перезавантаження конфігурації tmux

Після зміни `~/.config/tmux/tmux.conf` немає потреби перезапускати tmux-сесію.

Гаряча клавіша:

-   `Alt(Option)+r` - виконує:
...
### 8.8. Приклад робочого сценарію

1.  У вікні 1:
    

```text
Alt(Option)+v - розділити екран вертикально: зліва редактор, справа shell.
Alt(Option)+Down / Alt(Option)+Up / Alt(Option)+Left / Alt(Option)+Right - переміщатися між панелями.

```

2.  `Alt(Option)+Enter` - створити вікно 2: запустити backend-сервер, watcher, тести тощо.
    
3.  `Alt(Option)+3` - перейти у вікно 3: переглядати логи, tail файлів, моніторинг.
    
4.  За потреби від’єднатися від сесії: `Alt(Option)+d` - detach (усі процеси продовжують працювати).
...
Ghostty при цьому залишається “оболонкою” (вікна/таби macOS), а вся логіка layout’ів і сесій реалізована через tmux.
